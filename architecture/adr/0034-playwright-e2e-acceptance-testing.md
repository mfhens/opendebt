...
files. The agent reads `.factory/project.yaml` for `project.e2e.project_root`,
`project.e2e.test_dir`, `project.e2e.config`, and `project.e2e.base_urls`.
...
`playwright-test-generator` (not `bdd-test-generator`) when `.factory/project.yaml`
declares the Playwright E2E surface under `project.e2e`. The generator produces RED-phase
`test()` blocks — failing
...
- `.factory/project.yaml` — `project.e2e` defines `project_root`, `test_dir`, `config`, and `base_urls`
