...
| **E2E Playwright (ADR 0034)** | **Done** | petition029 — `opendebt-e2e/tests/petition029-creditor-claims.spec.ts`; petition030–038 — `opendebt-e2e/tests/petition030-038-creditor-portal-surfaces.spec.ts`; petition012 — `opendebt-e2e/tests/creditor-portal/petition012-bff-manual-submission.spec.ts` (template + GREEN BFF, full wizard submit to debt-service with demo CPR, act-as); shared helper `tests/helpers/creditor-portal-auth.ts`; CI adds `keycloak` to `/etc/hosts` for OAuth redirects |
