@petition057
Feature: Dækningsrækkefølge — GIL § 4 payment application order — rule-engine and API (P057)
  # Module scope: opendebt-payment-service (FR-1 through FR-7).
  # FR-8 (Sagsbehandlerportal view) is in opendebt-caseworker-portal petition057.feature.
  ...
  Scenario: Renter dækkes før Hovedfordring ved delvis betaling
    ...
    And Hoofdfordring receives no dækning
    And no dækning record has komponent "HOOFDFORDRING" with beloeb > 0
  ...
  Scenario: Fuld rentesekvens — alle seks under-positioner dækkes i korrekt rækkefølge
    ...
    And the Hoofdfordring receives 10.00 DKK dækning (the remaining amount after renter)
  ...
  Scenario: SKY-3029 Opskrivningsfordring — inddrivelsesrenter dækkes inden Hovedfordring ved delvis betaling (FR-5)
    ...
    # before its Hoofdfordring, following the same FR-3 interest sequence.
    ...
    And HOOFDFORDRING on fordring "FDR-30292" receives no dækning
