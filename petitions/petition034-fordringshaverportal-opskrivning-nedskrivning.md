...
- **G.A.1.4.3 (Opskrivning af opkrævningsrenter):** When the subject being adjusted is an opkrævningsrente (pre-collection interest that predates modtagelse), an opskrivningsfordring must NOT be used. Instead, a new rentefordring with the same renteperiode as the original claim must be submitted. The portal's OPSKRIVNING_REGULERING action type must validate this distinction and reject an opskrivning that targets a rentefordring (see also rule 474 in petition 017). The backend API must enforce this constraint even if the portal does not expose the choice.
- **G.A.1.4.4 (Retroactiv nedskrivning — reassignment obligation):** When a nedskrivning has a virkningsdato before the current date, Gældsstyrelsen (PSRM) must reassign all dækninger that occurred after the virkningsdato and recalculate interest for the affected period. The portal should inform fordringshavere that retroactive nedskrivninger (where virkningsdato precedes today) trigger this recalculation and may take additional processing time. Reference: G.A.1.4.4.
