...
- **Delt hæftelse scope:** PSRM only recognises solidarisk hæftelse in the inddrivelse system (G.A.1.3.3). For delt hæftelse, the fordringshaver must split the fordring into separate fordringer before submission — OpenDebt's inddrivelse layer must NOT model delt hæftelse internally. OpenDebt may model delt hæftelse for opkrævning-phase tracking only; the inddrivelse layer works exclusively on post-split fordringer submitted by the fordringshaver.
- **Hæftelsesstruktur changes on active claims:** A fordring that is under inddrivelse must be tilbagekaldt before its hæftelsesstruktur (which debtors are bound) can be changed. OpenDebt must not allow modification of hæftelse on a fordring in active inddrivelse without a preceding tilbagekald and re-submission. Reference: G.A.1.3.3 / GIL § 2, stk. 5.
