...
| P046-T4 | ✅ **Deprecate application.yml rate fallback** *(DONE 2026-03-21)*: Removed `@Value("${opendebt.interest.annual-rate}")` from `InterestRecalculationServiceImpl`, replaced with `BusinessConfigService.getDecimalValue("RATE_INDR_STD", from)` with fallback to 0.0575 on `ConfigurationNotFoundException`. Added `@PostConstruct validateRequiredConfigs()` to `BusinessConfigService` that warns on startup if table is empty. `annual-rate` property retained in `application.yml` with deprecation comment. | P046-T1, P046-T2 |
...
| Task ID | Description | Depends on |
|---------|-------------|------------|
| P047-T1 | ✅ **BusinessConfigEntity status extension + audit table migration** *(DONE 2026-03-21)*: Extended `BusinessConfigEntity` with `ReviewStatus` enum (PENDING_REVIEW, APPROVED) + `reviewStatus` field + `getComputedStatus(LocalDate)`/`getComputedStatus()` methods. Created `BusinessConfigAuditEntity` and `BusinessConfigAuditRepository`. Flyway V9 adds `review_status` column and `business_config_audit` table. | P046-T1 |
| P047-T2 | ✅ **BusinessConfigService CRUD extension** *(DONE 2026-03-21)*: Added DTOs (`CreateConfigRequest`, `UpdateConfigRequest`, `ConfigEntryDto`). Added repository queries (`findOverlapping`, `findOpenEnded`, `findAllDistinctKeys`). Added service methods: `createEntry` (validates past date, overlap, type, auto-closes previous open entry), `updateEntry` (PENDING_REVIEW/FUTURE only), `approveEntry`, `rejectEntry`, `deleteEntry`, `listAllGrouped`, `getEffectiveEntry`, `getAuditTrail`. All write ops audit-logged. | P047-T1 |
| P047-T3 | ✅ **Derived rate auto-computation** *(DONE 2026-03-21)*: When `RATE_NB_UDLAAN` is created, auto-generates RATE_INDR_STD (NB+4%), RATE_INDR_TOLD (NB+2%), RATE_INDR_TOLD_AFD (NB+1%) in PENDING_REVIEW. `createEntry` returns `ConfigCreationResult` with `created` + `derivedEntries`. Added `previewDerivedRates(nbRate, validFrom)` for side-effect-free preview. | P047-T2 |
| P047-T4 | ✅ **BusinessConfigController REST API** *(DONE 2026-03-21)*: Created `BusinessConfigController` at `/api/v1/config` with 8 endpoints: GET `/`, GET `/{key}`, GET `/{key}/history`, GET `/{key}/audit`, GET `/{key}/preview`, POST `/`, PUT `/{id}`, PUT `/{id}/approve`, PUT `/{id}/reject`, DELETE `/{id}`. Role-based `@PreAuthorize`. Exception handlers for `ConfigValidationException` (400) and `ConfigurationNotFoundException` (404). | P047-T2 |
| P047-T5 | ✅ **Unit tests for config API + service** *(DONE 2026-03-21)*: Created `BusinessConfigControllerTest` (11 tests), `BusinessConfigServiceCrudTest` (13 tests), `DerivedRateComputationTest` (6 tests). All 30 tests pass. JaCoCo coverage check passes. Full build SUCCESS. | P047-T4 |
...
### Sprint G: Config API foundation ✅ COMPLETED 2026-03-21

| # | Task | Est. | Status |
|---|------|------|--------|
| 1 | P046-T4 — Deprecate application.yml fallback | 30 min | ✅ Done |
| 2 | P047-T1 — Status extension + audit table migration | 45 min | ✅ Done |
| 3 | P047-T2 — BusinessConfigService CRUD extension | 60 min | ✅ Done |
| 4 | P047-T3 — Derived rate auto-computation | 45 min | ✅ Done |
