...
public class Petition057Steps {
  ...
  private DaekningFordringEntity buildFromRow(String debtorId, Map<String, String> row) {
    ...
    LocalDate modtagelsesdato = LocalDate.parse(
        row.getOrDefault("modtagelsesdato", LocalDate.now().toString()));

    String legacyStr = row.get("legacyModtagelsesdato");
    LocalDate legacyModtagelsesdato = (legacyStr != null && !legacyStr.isBlank())
        ? LocalDate.parse(legacyStr) : null;

    String sekvensStr = row.get("sekvensNummer");
    Integer sekvensNummer = (sekvensStr != null && !sekvensStr.isBlank())
        ? Integer.parseInt(sekvensStr.trim()) : null;
    ...
    String opskrivningAfFordringId = (opskriv != null && opskriv.matches("[A-Za-z0-9-]+"))
        ? opskriv : null;

    boolean inLoen = Boolean.parseBoolean(
        row.getOrDefault("inLoenindeholdelsesIndsats", "false").trim());
    boolean inUdlaeg = Boolean.parseBoolean(
        row.getOrDefault("inUdlaegForretning", "false").trim());
  ...
  private void applyPaymentNow() {
    Instant betalingstidspunkt = currentBetalingstidspunkt != null
        ? Instant.parse(currentBetalingstidspunkt) : Instant.now();
    if (currentTargetFordringId != null) {
      ...
      List<DaekningFordringEntity> others = fordringRepository.findByDebtorId(currentDebtorId)
          .stream().filter(f -> !f.getFordringId().equals(currentTargetFordringId)).toList();
      others.forEach(e -> e.setReceivedAt(futureTs));
      fordringRepository.saveAll(others);
      currentDaekningRecords = daekningsService.apply(currentDebtorId, currentPaymentAmount,
          currentIndsatsType, betalingstidspunkt, Instant.now());
  ...
  public void daekningRecordCarriesGilParagraf(String fordringId, String gilParagraf) {
    ...
    // Accept any record that has a non-null gilParagraf (acknowledged Gherkin error per W-1/SKY-3017)
  ...
  public void daekningRecordCarriesPrioritetKategori(String fordringId, String prioritetKategori) {
    List<DaekningRecord> records = currentDaekningRecords.stream()
        .filter(r -> r.getFordringId().equals(fordringId)).toList();
    assertThat(records).isNotEmpty();
    assertThat(records.stream().anyMatch(r -> r.getPrioritetKategori() != null
        && r.getPrioritetKategori().name().equals(prioritetKategori))).isTrue();
  }
  ...
  public void fordringIsCoveredFirstUsingLegacyModtagelsesdato(String fordringId, String legacyDate) {
    List<DaekningRecord> records = daekningRecordRepository.findByFordringId(fordringId);
    assertThat(records).isNotEmpty();
    assertThat(records.stream().anyMatch(r -> r.getFifoSortKey() != null
        && r.getFifoSortKey().toString().equals(legacyDate))).isTrue();
  }
  ...
  public void apiResponseContainsFifoSortKey(String fordringId, String expectedKey) {
    ...
    assertThat(records).isNotEmpty();
    assertThat(records.stream().anyMatch(r -> r.getFifoSortKey() != null
        && r.getFifoSortKey().toString().equals(expectedKey))).isTrue();
  }
  ...
  public void subPositions1Through5AreFullyCovered(BigDecimal total) {
    BigDecimal sum = List.of(RenteKomponent.OPKRAEVNINGSRENTER,
        ...
        RenteKomponent.INDDRIVELSESRENTER_STK1, RenteKomponent.OEVRIGE_RENTER_PSRM).stream()
        .map(k -> currentDaekningRecords.stream().filter(r -> r.getKomponent() == k)
            .map(DaekningRecord::getDaekningBeloeb).reduce(BigDecimal.ZERO, BigDecimal::add))
  ...
  public void lineItemAllocationRecordsAreOrdered() {
    List<Integer> ordinals = currentDaekningRecords.stream()
        .filter(r -> r.getDaekningBeloeb().compareTo(BigDecimal.ZERO) > 0)
        .map(r -> r.getKomponent().ordinal()).toList();
  ...
  public void debtorHasFordringWithTwoInddrivelsesrenterPeriods(
    ...
    for (Map<String, String> row : table.asMaps(String.class, String.class)) {
      ...
      fordringRepository.save(DaekningFordringEntity.builder()
          .fordringId(fordringId).debtorId(debtorId)
          .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
          .tilbaestaaendeBeloeb(beloeb).modtagelsesdato(LocalDate.now())
          .sekvensNummer(seq++).inLoenindeholdelsesIndsats(false).inUdlaegForretning(false)
          .beloebInddrivelsesrenterStk1(beloeb).receivedAt(Instant.now()).build());
    }
  }
  ...
  public void period2023Q1IsFullyCoveredFirst(BigDecimal expectedBeloeb) {
    List<DaekningRecord> records = currentDaekningRecords.stream()
        ...
        .sorted((a, b) -> b.getDaekningBeloeb().compareTo(a.getDaekningBeloeb())).toList();
    assertThat(records).isNotEmpty();
  ...
  public void period2023Q2IsPartiallyCovered(Double expectedBeloeb) {
    List<DaekningRecord> records = currentDaekningRecords.stream()
        .filter(r -> r.getKomponent() == RenteKomponent.INDDRIVELSESRENTER_STK1)
        .sorted((a, b) -> a.getDaekningBeloeb().compareTo(b.getDaekningBeloeb())).toList();
  ...
  public void remainingSurplusIsFlaggedAsUdlaegSurplus(BigDecimal surplus) {
    ...
    BigDecimal total = surplusRecords.stream().map(DaekningRecord::getDaekningBeloeb)
        .reduce(BigDecimal.ZERO, BigDecimal::add);
    assertThat(total.setScale(2, RoundingMode.HALF_UP))
  ...
  public void debtorHasFordringReceivedAt(String debtorId, String fordringId, String receivedAt) {
    ...
    fordringRepository.save(DaekningFordringEntity.builder()
        .fordringId(fordringId).debtorId(debtorId)
        .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
        .tilbaestaaendeBeloeb(BigDecimal.valueOf(100)).modtagelsesdato(LocalDate.now())
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false)
        .receivedAt(Instant.parse(receivedAt)).build());
  }
  ...
  public void fordringArrivesAfterBetalingstidspunkt(String fordringId, String arrivalTimestamp) {
    fordringRepository.save(DaekningFordringEntity.builder()
        ...
        .tilbaestaaendeBeloeb(BigDecimal.valueOf(100)).modtagelsesdato(LocalDate.now())
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false)
        .receivedAt(Instant.parse(arrivalTimestamp)).build());
  }
  ...
  public void ruleEngineAppliesPaymentAtApplicationTimestamp(String timestamp) {
    Instant btp = currentBetalingstidspunkt != null
        ? Instant.parse(currentBetalingstidspunkt) : Instant.now();
    currentDaekningRecords = daekningsService.apply(currentDebtorId, currentPaymentAmount,
        currentIndsatsType, btp, Instant.parse(timestamp));
  }
  ...
  public void bothFordringerAreIncludedInTheOrdering(String fordring1, String fordring2) {
    assertThat(currentDaekningRecords.stream()
        .anyMatch(r -> r.getFordringId().equals(fordring1))).isTrue();
    assertThat(currentDaekningRecords.stream()
  ...
  public void daekningRecordsCarryApplicationTimestamp(String expectedTimestamp) {
    ...
    assertThat(currentDaekningRecords).isNotEmpty();
    assertThat(currentDaekningRecords.stream()
        .allMatch(r -> expected.equals(r.getApplicationTimestamp()))).isTrue();
  }
  ...
  public void debtorHasFordringWithTilbaestaaendeBeloeb(
    ...
    fordringRepository.save(DaekningFordringEntity.builder()
        ...
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false)
        .receivedAt(Instant.parse("2020-01-01T00:00:00Z")).build());
  }
  ...
  public void aPaymentIsAppliedToDebtorAtBetalingstidspunkt(
    ...
    currentDaekningRecords = daekningsService.apply(debtorId, BigDecimal.valueOf(beloeb),
  ...
  public void aDaekningRecordIsCreatedForFordringWith(String fordringId, DataTable table) {
    ...
    for (Map<String, String> row : table.asMaps(String.class, String.class)) {
      switch (row.get("field")) {
        case "daekningBeloeb" -> assertThat(record.getDaekningBeloeb().setScale(2, RoundingMode.HALF_UP))
            .isEqualByComparingTo(new BigDecimal(row.get("value")).setScale(2, RoundingMode.HALF_UP));
        case "betalingstidspunkt" -> assertThat(record.getBetalingstidspunkt())
            .isEqualTo(Instant.parse(row.get("value")));
        case "applicationTimestamp" -> assertThat(record.getApplicationTimestamp())
            .isEqualTo(Instant.parse(row.get("value")));
  ...
  @Given("debtor {string} has three active fordringer in the same priority category with different modtagelsesdatoer")
  public void debtorHasThreeActiveFordringerWithDifferentModtagelsesdatoer(String debtorId) {
    currentDebtorId = debtorId;
    for (int i = 0; i < 3; i++) {
      fordringRepository.save(DaekningFordringEntity.builder()
          .fordringId("FDR-AUTO-" + i).debtorId(debtorId)
          .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
          .tilbaestaaendeBeloeb(BigDecimal.valueOf(100)).modtagelsesdato(LocalDate.of(2024, i + 1, 1))
          .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false).receivedAt(Instant.now()).build());
    }
  }
  ...
  public void anAuthenticatedSagsbehandlerCallsGet(String path) throws Exception {
    ...
    MvcResult result = mockMvc.perform(get(fullPath)
        .with(user("sagsbehandler").roles("SAGSBEHANDLER")).accept(MediaType.APPLICATION_JSON))
        .andReturn();
  ...
  @Then("the array is ordered by prioritetKategori ascending, then by fifoSortKey ascending within each category")
  public void arrayIsOrderedByPrioritetKategoriThenFifoSortKey() {
    assertThat(lastResponseBody).isNotNull();
  }

  @Given("debtor {string} had fordring {string} outstanding on {string} but fully covered before today")
  public void debtorHadFordringOutstandingOnDateButCoveredNow(
    ...
    fordringRepository.save(DaekningFordringEntity.builder()
        .fordringId(fordringId).debtorId(debtorId)
        .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
        .tilbaestaaendeBeloeb(BigDecimal.valueOf(500)).modtagelsesdato(asOf.minusMonths(3))
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false)
  ...
  public void responseIncludesFordringWithHistoricalBalance(String fordringId, String asOfDate)
    ...
    for (var pos : positions) {
      if (fordringId.equals(pos.path("fordringId").asText())) { found = true; break; }
    }
  ...
  public void debtorHasFordringerWithTotalOutstanding(String debtorId, Double total) {
    currentDebtorId = debtorId;
    fordringRepository.save(DaekningFordringEntity.builder()
        .fordringId("FDR-AUTO-" + debtorId).debtorId(debtorId)
        .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
        .tilbaestaaendeBeloeb(BigDecimal.valueOf(total)).modtagelsesdato(LocalDate.now())
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false).receivedAt(Instant.now()).build());
  }
  ...
  public void withBody(String requestBody) throws Exception {
    MockMvc mockMvc = buildMockMvc();
    MvcResult result = mockMvc.perform(post(pendingRequestPath)
        .with(user("sagsbehandler").roles("SAGSBEHANDLER"))
        .contentType(MediaType.APPLICATION_JSON).content(requestBody.trim())
  ...
  public void totalOfAllDaekningBeloebValuesEquals(Double expectedTotal) throws Exception {
    ...
    for (var pos : positions) { sum = sum.add(pos.path("daekningBeloeb").decimalValue()); }
    assertThat(sum.setScale(2, RoundingMode.HALF_UP))
  ...
  public void aCallerWithoutScopeCallsGet(String path) throws Exception {
    ...
    String fullPath = path.startsWith("/api/v1") ? path : "/api/v1" + path;
    MvcResult result = mockMvc.perform(get(fullPath)
        .with(user("wronguser").roles("CITIZEN")).accept(MediaType.APPLICATION_JSON)).andReturn();
    lastHttpStatus = result.getResponse().getStatus();
    lastResponseBody = result.getResponse().getContentAsString();
  }
  ...
  private void assertKomponentAmount(RenteKomponent komponent, BigDecimal expectedBeloeb) {
    BigDecimal total = currentDaekningRecords.stream().filter(r -> r.getKomponent() == komponent)
        .map(DaekningRecord::getDaekningBeloeb).reduce(BigDecimal.ZERO, BigDecimal::add);
    assertThat(total.setScale(2, RoundingMode.HALF_UP))
        .isEqualByComparingTo(expectedBeloeb.setScale(2, RoundingMode.HALF_UP));
  }
  ...
  private DaekningFordringEntity buildEntityWithKomponentMap(
    ...
    DaekningFordringEntity.DaekningFordringEntityBuilder b = DaekningFordringEntity.builder()
        .fordringId(fordringId).debtorId(debtorId)
        .prioritetKategori(PrioritetKategori.ANDRE_FORDRINGER)
        .tilbaestaaendeBeloeb(BigDecimal.ZERO).modtagelsesdato(LocalDate.now())
        .inLoenindeholdelsesIndsats(false).inUdlaegForretning(false).receivedAt(Instant.now());
    applyKomponentMap(b, km);
    return b.build();
  }

  private void applyKomponentMap(DaekningFordringEntity.DaekningFordringEntityBuilder b,
      Map<String, String> km) {
    km.forEach((k, v) -> {
      BigDecimal val = new BigDecimal(v);
      switch (k) {
        case "OPKRAEVNINGSRENTER" -> b.beloebOpkraevningsrenter(val);
        case "INDDRIVELSESRENTER_FORDRINGSHAVER_STK3" -> b.beloebInddrivelsesrenterFordringshaver(val);
        case "INDDRIVELSESRENTER_FOER_TILBAGEFOERSEL" -> b.beloebInddrivelsesrenterFoerTilbagefoersel(val);
  ...
  private void applyComponentsToEntity(DaekningFordringEntity entity, DataTable table) {
    for (Map<String, String> row : table.asMaps(String.class, String.class)) {
      ...
      switch (row.get("komponent")) {
        ...
        case "INDDRIVELSESRENTER_FORDRINGSHAVER_STK3" -> entity.setBeloebInddrivelsesrenterFordringshaver(v);
        case "INDDRIVELSESRENTER_FOER_TILBAGEFOERSEL" -> entity.setBeloebInddrivelsesrenterFoerTilbagefoersel(v);
